...
public class BracketsCellAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    private void setFields(final BracketsCellViewHolder viewHolder, final int position) {
        ...
        Glide.with(context).load(list.get(position).getCompetitorOne().getUrl()).into(viewHolder.getTeamOneImageview());
        Glide.with(context).load(list.get(position).getCompetitorTwo().getUrl()).into(viewHolder.getTeamTwoImageview());
